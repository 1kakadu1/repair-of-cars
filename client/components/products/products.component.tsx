...
export const ProductsGrid = ({}: IProductsProps)=>{
    ...
    const onChangeOrder = (value: any)=>{
        dispatch(toProductsAction.changeProductsOptions({
            ...options,
            orderBy: value,
        }))
        dispatch(fetchProductsList(
    ...
    return(
        <div className={cl.products}>
            {
                products.length === 0 ? 
                <div className={cl.productsEmpty}>По данному запросу найдено 0 продуктов</div>
                :
                (
                    <>
                        <div className={cl.productsHeader}>
                            ...
                            />
                            <div className={cl.productsTotal}>
                                Товаров: {total}
                            </div>
                        </div>
                        <div className={`container__row ${cl.popularRowMgTop}`}>
                            {
                                products.map(item =>(
                                    <div className={`container__col-12 container__col-md-6 container__col-xl-4 container__col--stretch`} key={item.id}>
                                        <CardProduct
                                            product={item}
                                            onAdd={onAddItem}
