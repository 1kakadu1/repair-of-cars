...
const MenuSub = ({ links, open, active, onClose }: IMenuSubProps) => {
	return (
		<PortalWrapper wrapperId="react-portal-menu">
			<div
				className={`${styles.menu__full} ${
					open ? styles.menu__full_active : ''
				}`}
			>
							...
							.map((parent) => {
								const itemsPreview =
									parent.subMenu?.filter(
										(item) => item.preview !== undefined && item.preview !== ''
									) || [];
								const itemsDefault =
									parent.subMenu?.filter(
										(item) => item.preview === undefined || item.preview === ''
								...
								return (
									<div
										className={`${styles.menu__full__parent}  ${
											active === parent.href
												? styles.menu__full__parent_show
												: ''
										}`}
									...
									>
										...
										<div
											className={
												styles.menu__full__parent__item +
												' ' +
												(itemsPreview.length > 0
													? styles.menu__full__parent__item_pdLeft
													: '')
											}
										>
...
export const Menu = ({ links }: IMenuProps) => {
	...
	return (
		<div className={styles.menu}>
			...
			<div
				className={
					styles.menu__list__wrap +
					' ' +
					(mobileMenu ? styles.menu__list__wrap__mobile__active : '')
				}
			>
				<ul className={styles.menu__list + ' container'}>
					{links.map((item) => (
						<li
							...
							key={item.href}
							onClick={() => (item.subMenu ? onToggle(item.href) : 0)}
						>
