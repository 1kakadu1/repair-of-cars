...
import NotificationContext, { NotificationStatus } from "../../notification-bar/notification-bar.context";
...
export const ModalCallback = ({onSend: onSendProps, open, onToggleOpen}:IModalCallbackProps) =>{
	...
	const onSend = () => {
		...
		if (values.phone.touch && values.name.touch) {
			if (values.name.valid && values.phone.valid) {
                ...
                onSendProps && onSendProps({name: values.name.value, phone: values.phone.value });
				updateNotification({
