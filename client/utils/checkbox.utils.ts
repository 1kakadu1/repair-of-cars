...
export function toggleSelectedItems<T = string>({ list, item, index }: { list: T[]; item: T; index: number }): T[] {
    let newSelected: T[] = [];
    ...
    if (index === -1) {
      ...
      newSelected = newSelected.concat(list.slice(0, index), list.slice(index + 1));
    }
