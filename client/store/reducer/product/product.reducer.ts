...
const setProduct = (
	...
	state.product = !Array.isArray(payload.products)
		? payload.products
		: state.product;
};
...
export const fetchProductBySlug = createAsyncThunk(
	...
	async (
		...
		const response = await apiService.get<IProductReqData>(`products/` + slug, {
			...body,
			...options,
		});
...
export const productSlice = createSlice({
	...
	extraReducers: {
		...
		[fetchProductBySlug.fulfilled.type]: (
			state: IProductState,
			{ payload }: { payload: IProductReqData }
		) => {
			state.product = !Array.isArray(payload.products)
		...
		[fetchProductSimilarList.fulfilled.type]: (
			state: IProductState,
			{ payload }: { payload: IProductReqData }
		) => {
			state.similar.products = Array.isArray(payload.products)
				? payload.products
		...
		[fetchProductSimilarList.rejected.type]: (
			state: IProductState,
			{ payload }
		) => {
