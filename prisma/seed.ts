...
import { mockNewsDescription, mockProductDescription } from './data/mock';
...
const categoriesData: Prisma.CategoryCreateInput[] = [
	...
	},
	{
		...
		description:
			'Летние Зимние Шипованые Фрикционные Б/у. R13 R14 R15 R16 R17 R18 R19 R20',
		published: true,
	},
	{
		id: 'category-cargo',
		...
		description:
			'Ведущие Универсальные Рулевые Прицеп Рулевые + прицеп. R22,5 R24 R21 R20 R19,5 R18 R19 R20',
		published: true,
	},
	{
		...
		description:
			'Спорт Спорт-турист Чоппер/круйзер Эндуро Классика. R12 R13 R14 R15',
		published: true,
	},
	{
		id: 'category-news',
...
const newsData: Prisma.NewsCreateInput[] = [
	{
		...
		isStock: true,
		description: mockNewsDescription({ ol: true, table: true }),
	},
	{
		id: 'news-2',
		...
		tags: '["Диски", "Шины"]',
		isStock: true,
		description: mockNewsDescription({ li: true }),
	},
	{
		id: 'news-3',
		...
		published: true,
		description: mockNewsDescription({ ol: true }),
		users: {
			create: {
				email: 'del@del.ru',
				password: '12345678',
	...
	},
	{
		...
		preview: '/img/news/news-4.png',
		published: true,
		description: mockNewsDescription({ ol: true, li: true, table: true }),
		users: {
			create: {
				email: 'del@del.ru',
				password: '12345678',
		...
		slug: 'news-orders-work',
		isStock: false,
		tags: '["Приём на работу"]',
	},
];
...
async function main() {
	...
	for (const key in dictionaries) {
		for (const u of dictionaries[key]) {
			if (prisma[key as never] !== undefined) {
				...
				console.log(
					`Created dictionaries ${key} with id: ${dictionariesItem.id}`
				);
			} else {
	...
	if (admin) {
		...
		for (const u of communications.catRefServices) {
			...
			console.log(
				`Create ref Service to Cat services with id: ${
					createRefCat.categoryId + '=' + createRefCat.serviceId
				}`
			);
		}
		...
		for (const u of communications.catRefNews) {
			...
			console.log(
				`Create ref News to Cat news with id: ${
					createRefCat.categoryId + '=' + createRefCat.newsId
				}`
			);
		}
		...
		for (const u of communications.catRefProducts) {
			...
			console.log(
				`Create ref Product to Cat products with id: ${
					createRefCat.categoryId + '=' + createRefCat.productId
				}`
			);
		}
