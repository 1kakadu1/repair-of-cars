...
import { fetchProductBySlug, fetchProductSimilarList, toProductAction } from '../../client/store/reducer/product/product.reducer';
...
import { useWindowSize, WindowBreakpoints } from '../../client/hooks/useWindowSize';
...
const Product: NextPage = () => {
  ...
  const slug = Array.isArray(query.slug) ? query.slug[0] : query.slug || "unset";
  ...
  useEffect(()=>{
    ...
    if(slug !== "unset" && !Array.isArray(slug) && slug !== ""){
      ...
      if(productStore){
        dispatch(toProductAction.setProduct({
      ...
      if(!similar.isLoading){
       dispatch(fetchProductSimilarList({body: {slug}, options: {limit: 4, offset: 0}}) as any);
      }
    }
  ...
  const tabItems = [
		product ? (
			<div className={cl.pageDescription} dangerouslySetInnerHTML={{__html: product.description}} />
		) : (
			<div />
		),
    product ? (
                  ...
                  <div className={cl.pageHeader}>
                    ...
                    <div className={cl.pageBody}>
                      <div className={`container__row`}>
                        <div className={`container__col-12 container__col-md-6  container__col-xl-4 ${cl.pageBodyPreview}`}>
                          <Image
                              layout='fill'
                        ...
                        </div>
                        {
                          width >= WindowBreakpoints.xl && (
                            <div className={`container__col-12 container__col-xl-4 ${cl.pageBodyInfo}`}>
                              <TextItems 
                        ...
                        <div className={`container__col-12 container__col-md-6 container__col-xl-4 ${cl.pageBodyCart}`}>
                            <CardActionProduct
                              product={product}
                            />
                        </div>
